<main class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary-dark">Contact Form Submissions</h2>
    </div>
            ...
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs uppercase">Message</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">Submitted On</th>
                    </tr>
                </thead>

                <tbody>
                    <?php if (!empty($contact_forms)): ?>
                        <?php foreach ($contact_forms as $index => $item): ?>
                            <tr>
                                <td class="px-6 py-4"><?= $index + 1 ?></td>
                                <td class="px-6 py-4"><?= esc($item->full_name) ?></td>
                                <td class="px-6 py-4"><?= esc($item->email) ?></td>
                                <td class="px-6 py-4"><?= esc($item->phone) ?></td>
                                <td class="px-6 py-4"><?= esc($item->subject) ?></td>
                                <td class="px-6 py-4"><?= esc($item->message) ?></td>
                                <td class="px-6 py-4"><?= esc($item->created_at) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
