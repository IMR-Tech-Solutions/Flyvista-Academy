<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary-dark">Success Stories Section</h2>
        <a href="<?= base_url('admin/success-stories/add') ?>"
            class="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg flex items-center">
            <i class="fas fa-plus mr-2"></i> Add Success Story
        </a>
    </div>
            ...
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs uppercase">Featured</th>
                ...
                <tbody>
                    <?php if (!empty($stories)): ?>
                        <?php foreach ($stories as $index => $item): ?>
                            <tr>
                                <td class="px-6 py-4"><?= $index + 1 ?></td>
                                <td class="px-6 py-4"><?= esc($item->name) ?></td>
                                <td class="px-6 py-4"><?= esc($item->role) ?></td>
                                <td class="px-6 py-4"><?= $item->is_featured ? '<span class="text-green-600 font-semibold">Yes</span>' : 'No' ?></td>
                                <td class="px-6 py-4 text-left">
                                    <a href="<?= base_url('admin/success-stories/edit/' . $item->id) ?>"
                                    ...
                                    <a href="<?= base_url('admin/success-stories/delete/' . $item->id) ?>"
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="px-6 py-4 text-center text-gray-500">No success stories found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
