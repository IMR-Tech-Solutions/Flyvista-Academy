...
<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary-dark">Blog Posts</h2>
        <a href="<?= base_url('admin/blogs/add') ?>"
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-4 flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            ...
            <div class="flex items-center max-w-xs w-full">
                <label for="searchBlog" class="sr-only">Search</label>
                <div class="relative w-full">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <i class="fas fa-search text-gray-400"></i>
                    </div>
                    <input type="text" id="searchBlog" placeholder="Search Title..."
            ...
            <div class="flex items-center space-x-2">
                <label for="lengthMenuBlog" class="font-medium text-gray-700">Show entries:</label>
                <select id="lengthMenuBlog"
            ...
            <!-- Export Buttons -->
            <div class="flex items-center space-x-2">
                <button id="exportBlogExcel" class="px-3 py-1 rounded-lg hover:bg-primary/20 bg-secondary/30 text-xl"
                    title="Download Excel">
                    <i class="fas fa-file-csv text-green-600"></i>
                </button>
                <button id="exportBlogPdf" class="px-3 py-1 rounded-lg hover:bg-primary/20 bg-secondary/30 text-xl"
                    title="Download PDF">
        ...
        <div class="table-responsive px-4 pb-4">
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs uppercase">Category</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">Date</th>
                ...
                <tbody>
                    <?php foreach ($blogs as $index => $item): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4"><?= esc($item->category) ?></td>
                            <td class="px-6 py-4"><?= esc($item->post_date) ?></td>
                            <td class="px-6 py-4 text-left">
                                <a href="<?= base_url('admin/blogs/edit/' . $item->id) ?>"
                                ...
                                <a href="<?= base_url('admin/blogs/delete/' . $item->id) ?>"
            ...
            <div id="blogPageInfo" class="text-sm text-gray-600 mt-2"></div>
        </div>
    </div>
</main>