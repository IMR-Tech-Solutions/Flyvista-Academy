...
<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary-dark">Career Features</h2>
        <a href="<?= base_url('admin/career-features/add') ?>"
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div
            ...
            <div class="flex items-center max-w-xs w-full">
                <label for="searchCareerFeature" class="sr-only">Search</label>
                <div class="relative w-full">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <i class="fas fa-search text-gray-400"></i>
                    </div>
                    <input type="text" id="searchCareerFeature" placeholder="Search Title..."
            ...
            <div class="flex items-center space-x-2">
                <label for="lengthMenuCareerFeature" class="font-medium text-gray-700">Show entries:</label>
                <select id="lengthMenuCareerFeature"
            ...
            <!-- Export Buttons -->
            <div class="flex items-center space-x-2">
                <button id="exportCareerFeatureExcel" class="px-3 py-1 rounded-lg hover:bg-primary/20 bg-secondary/30 text-xl"
                    title="Download Excel">
                    <i class="fas fa-file-csv text-green-600"></i>
                </button>
                <button id="exportCareerFeaturePdf" class="px-3 py-1 rounded-lg hover:bg-primary/20 bg-secondary/30 text-xl"
                    title="Download PDF">
        ...
        <div class="table-responsive px-4 pb-4">
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                ...
                <tbody>
                    <?php foreach ($career_features as $index => $item): ?>
                        <tr>
                            <td class="px-6 py-4"><?= $index + 1 ?></td>

                            <td class="px-6 py-4">
                                <i class="<?= esc($item->icon) ?> text-primary text-lg"></i>
                            </td>
                            ...
                            <td class="px-6 py-4 text-left">
                                <a href="<?= base_url('admin/career-features/edit/' . $item->id) ?>"
                                ...
                                <a href="<?= base_url('admin/career-features/delete/' . $item->id) ?>"
            ...
            <div id="careerFeaturePageInfo" class="text-sm text-gray-600 mt-2"></div>
        </div>
    </div>
</main>