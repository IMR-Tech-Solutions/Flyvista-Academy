<main class="p-6">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary-dark">Contact Messages</h2>
    </div>
            ...
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                ...
                <tbody>
                    <?php if (!empty($contact_messages)): ?>
                        <?php foreach ($contact_messages as $index => $msg): ?>
                            <tr>
                                <td class="px-6 py-4"><?= $index + 1 ?></td>
                                <td class="px-6 py-4"><?= esc($msg->name) ?></td>
                                <td class="px-6 py-4"><?= esc($msg->email) ?></td>
                                <td class="px-6 py-4"><?= esc($msg->phone) ?></td>
                                <td class="px-6 py-4"><?= esc($msg->course) ?></td>
                                <td class="px-6 py-4"><?= esc($msg->message) ?></td>
                                <td class="px-6 py-4"><?= esc($msg->created_at) ?></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
