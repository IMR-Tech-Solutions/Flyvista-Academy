...
<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary">Admission Process</h2>
        <a href="<?= base_url('admin/admission/add') ?>"
            ...
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs uppercase">Step Number</th>
                ...
                <tbody>
                    <?php foreach ($admission_process as $index => $item): ?>
                        <tr>
                            <td class="px-6 py-4"><?= $index + 1 ?></td>
                            <td class="px-6 py-4"><?= esc($item->step_number) ?></td>
                            <td class="px-6 py-4"><?= esc($item->title) ?></td>
                            <td class="px-6 py-4"><i class="<?= esc($item->icon) ?>"></i></td>

                            <td class="px-6 py-4 text-left">
                                <a href="<?= base_url('admin/admission/edit/' . $item->id) ?>"
                                ...
                                <a href="<?= base_url('admin/admission/delete/' . $item->id) ?>"
