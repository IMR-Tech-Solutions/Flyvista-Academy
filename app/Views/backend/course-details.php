...
<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary-dark">Course Details</h2>
        <a href="<?= base_url('admin/course-details/add') ?>"
            class="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg flex items-center">
            <i class="fas fa-plus mr-2"></i> Add Course Details
        </a>
    </div>
            ...
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                ...
                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs uppercase">Course Title</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">About Title</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">Total Skills</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">Total Training Methods</th>
                ...
                <tbody>
                    <?php foreach ($details as $index => $item): ?>
                        ...
                        <?php
                            ...
                            $training = json_decode($item->training_methods ?? '[]', true);
                        ?>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <?= esc($item->course_title ?? 'N/A') ?>
                            </td>

                            <td class="px-6 py-4"><?= esc($item->about_title) ?></td>

                            <td class="px-6 py-4"><?= count($skills) ?></td>

                            <td class="px-6 py-4"><?= count($training) ?></td>

                            <td class="px-6 py-4 text-left">
                                <a href="<?= base_url('admin/course-details/edit/' . $item->id) ?>"
                                ...
                                <a href="<?= base_url('admin/course-details/delete/' . $item->id) ?>"
