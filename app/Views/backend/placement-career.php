...
<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary">Placement & Career Development</h2>
        <a href="<?= base_url('admin/placement-career/add') ?>"
            class="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg flex items-center">
            <i class="fas fa-plus mr-2"></i> Add Placement Section
        </a>
    </div>
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-4 flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            ...
            <div class="flex items-center max-w-xs w-full">
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" id="search" placeholder="Search Heading..."
            ...
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs uppercase">Sub Heading</th>
                ...
                <tbody>
                    <?php foreach ($placement as $index => $item): ?>
                        <tr>
                            <td class="px-6 py-4"><?= $index + 1 ?></td>

                            <td class="px-6 py-4"><?= esc($item->section_heading) ?></td>

                            <td class="px-6 py-4"><?= esc($item->section_subheading) ?></td>

                            <td class="px-6 py-4">
                                <?php if ($item->status == 1): ?>
                                    <span class="px-3 py-1 bg-green-200 text-green-800 rounded text-xs font-semibold">Active</span>
                                <?php else: ?>
                                    <span class="px-3 py-1 bg-red-200 text-red-800 rounded text-xs font-semibold">Inactive</span>
                                <?php endif; ?>
                            </td>

                            <td class="px-6 py-4 text-left">
                                <a href="<?= base_url('admin/placement-career/edit/' . $item->id) ?>"
                                ...
                                <a href="<?= base_url('admin/placement-career/delete/' . $item->id) ?>"
