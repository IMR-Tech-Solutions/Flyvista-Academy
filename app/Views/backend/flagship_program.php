...
<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary">Flagship Programs</h2>
        <a href="<?= base_url('admin/flagship_program/add') ?>"
            class="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg flex items-center">
            <i class="fas fa-plus mr-2"></i> Add Flagship Program
        </a>
    </div>
            ...
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs uppercase">Subtitle</th>
                ...
                <tbody>
                    <?php foreach ($programs as $index => $program): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4"><?= esc($program->title) ?></td>
                            <td class="px-6 py-4"><?= esc($program->subtitle) ?></td>
                            <td class="px-6 py-4 text-left">
                                <a href="<?= base_url('admin/flagship_program/edit/' . $program->id) ?>"
                                ...
                                <a href="<?= base_url('admin/flagship_program/delete/' . $program->id) ?>"
