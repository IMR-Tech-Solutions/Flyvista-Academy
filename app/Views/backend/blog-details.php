...
<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary-dark">Blog Details</h2>

        <a href="<?= base_url('admin/blog-details/add') ?>"
            class="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg flex items-center">
            <i class="fas fa-plus mr-2"></i> Add Blog Details
        </a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-4 
            ...
            <div class="flex items-center max-w-xs w-full">
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" id="search" placeholder="Search Blog Title..."
            ...
            <div class="flex items-center space-x-2">
                <label class="font-medium text-gray-700">Show entries:</label>
        ...
        <div class="table-responsive px-4 pb-4">
            <table id="servicesTable"
                class="table table-striped table-bordered w-full text-sm md:text-base">

                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs uppercase">Sr No.</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">Blog Title</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">Aviation Facts</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">TOC</th>
                ...
                <tbody>
                    <?php foreach ($details as $index => $item): ?>
                        ...
                        <?php
                            $facts = json_decode($item->aviation_fact ?? '[]', true);
                            $toc = json_decode($item->table_of_contents ?? '[]', true);
                        ?>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4">
                                <?= esc($item->blog_title ?? 'N/A') ?>
                            </td>

                            <td class="px-6 py-4"><?= count($facts) ?></td>
                            <td class="px-6 py-4"><?= count($toc) ?></td>

                            <td class="px-6 py-4 text-left">
                                <a href="<?= base_url('admin/blog-details/edit/' . $item->id) ?>"
                                ...
                                <a href="<?= base_url('admin/blog-details/delete/' . $item->id) ?>"
