...
<main class="p-6">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-secondary-dark">Counters</h2>
        <a href="<?= base_url('admin/counters/add') ?>"
            class="bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-lg flex items-center">
            <i class="fas fa-plus mr-2"></i> Add New Counter
        </a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-4 flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
            ...
            <div class="flex items-center max-w-xs w-full">
                <label for="searchCounter" class="sr-only">Search</label>
                <div class="relative w-full">
                    <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <i class="fas fa-search text-gray-400"></i>
                    </div>
                    <input type="text" id="searchCounter" placeholder="Search Title..."
            ...
            <div class="flex items-center space-x-2">
                <label for="lengthMenuCounter" class="font-medium text-gray-700">Show entries:</label>
                <select id="lengthMenuCounter"
            ...
            <!-- Export Buttons -->
            <div class="flex items-center space-x-2">
                <button id="exportCounterExcel" class="px-3 py-1 rounded-lg hover:bg-primary/20 bg-secondary/30 text-xl"
                    title="Download Excel">
                    <i class="fas fa-file-csv text-green-600"></i>
                </button>
                <button id="exportCounterPdf" class="px-3 py-1 rounded-lg hover:bg-primary/20 bg-secondary/30 text-xl"
                    title="Download PDF">
        ...
        <div class="table-responsive px-4 pb-4">
            <table id="servicesTable" class="table table-striped table-bordered w-full text-sm md:text-base">
                <thead class="bg-gray-200 border border-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs uppercase">Icon</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">Count</th>
                        <th class="px-6 py-3 text-left text-xs uppercase">Title</th>
                ...
                <tbody>
                    <?php foreach ($counters as $index => $item): ?>
                        <tr>
                            ...
                            <td class="px-6 py-4"><i class="<?= esc($item->icon) ?> text-primary text-lg"></i></td>
                            <td class="px-6 py-4"><?= esc($item->count) ?></td>
                            <td class="px-6 py-4"><?= esc($item->title) ?></td>
                            <td class="px-6 py-4 text-left">
                                <a href="<?= base_url('admin/counters/edit/' . $item->id) ?>"
                                ...
                                <a href="<?= base_url('admin/counters/delete/' . $item->id) ?>"
            ...
            <div id="counterPageInfo" class="text-sm text-gray-600 mt-2"></div>
        </div>
    </div>
</main>